    ...
    dashboardSidebar(
      ...
      sidebarMenu(id = "sbm",
                  ...
                  conditionalPanel("input.sbm === 'dac'",
                                   selectizeInput("startDataset", "Dataset", c("","esoph", "upload my own"), multiple = F),
                                   ...
                                   conditionalPanel("input.clean === true", 
                                                    ...
                                                    conditionalPanel("input.combine === true",
                                                                     ...
                                                                     div(style="text-align: center", actionButton("mergeButton", "Merge Data"))),
                                                    selectizeInput("cleanOptions", "Cleaning Options", c("",  "replace values", "replace NAs", "remove NAs" )), 
                                                    conditionalPanel("input.cleanOptions === 'replace NAs'", textInput("replacementNA", "Replacement NA Value", "")),
                                                    conditionalPanel("input.cleanOptions === 'replace values'", selectizeInput("replaceValues", "Replacement Options", c("values in entire dataset", "values in columns")),
                  ...
                  conditionalPanel("input.sbm === 'crs'", "Response Data",
                                   ...
                                   conditionalPanel("input.corrSig === true", numericInput("sigLevel", "Significance Level", 0.05, 0, 1, 0.01), 
                                                    selectizeInput("inSig", "Insignificant Action", c("pch", "p-value", "blank", "n"))),
                  ...
                  menuItem("Regression and Survival Data", tabName = "rsd", icon = icon("th")),
    ...
    dashboardBody(
      ...
      tabItems(
                ...
                fluidRow(
                  column(width = 3, 
                         box(title = "Data Overview Options", status = "warning", collapsible = TRUE, width = NULL, solidHeader = TRUE, 
                             selectizeInput("look", "Look at NAs, Characters, and Numeric values in Dataset", c("Total Dataset", "NAs By Row", "Data Type By Column"))), 
        ...
        tabItem(tabName = "pst",
                ...
                fluidRow(
                  box(uiOutput("plot1"), status = "primary", collapsiable = TRUE, solidHeader = FALSE),
                  box(uiOutput("plot2"), status = "primary", collapsiable = TRUE, solidHeader = FALSE)
                ),
                fluidRow(
                  box(title = "Plot 1 Labels", status = "primary", collapsible = TRUE, collapsed = TRUE, width = 6, textInput("xAxisLabel", "x-axis", value = ""), textInput("yAxisLabel", "y-axis", value = ""), 
                      textInput("title", "Title", value = ""), textInput("legend", "Legend", value = ""), textInput("legendLabels1", "Legend Labels", value = ""), selectizeInput("fill", "Color Pallette", c('Set1', 'Set3','Blues', 'Solid Blue', 'BuGn', 'BuPu', 'GnBu', 'Greens','Solid Grey', 'Greys', 'Paired', 'Spectral', 'Solid Red', 'RdBu', 'RdGy'))
                  ), 
                  box(title = "Plot 2 Labels", status = "primary", collapsible = TRUE, collapsed = TRUE, width = 6, textInput("xAxisLabel2", "x-axis", value = ""), textInput("yAxisLabel2", "y-axis", value = ""), 
                      textInput("title2", "Title", value = ""), textInput("legend2", "Legend", value = ""), textInput("legendLabels2", "Legend Labels", value = ""), selectizeInput("fill2", "Color Pallette", c('Set1', 'Set3','Blues', 'Solid Blue', 'BuGn', 'BuPu', 'GnBu', 'Greens','Solid Grey', 'Greys', 'Paired', 'Spectral', 'Solid Red', 'RdBu', 'RdGy'))
                  )
                ),
                fluidRow(
        ...
        tabItem(tabName = "crs",
                ...
                fluidRow(
                  box(title = "Regression Output", status = "primary", collapsible = TRUE, width = 10, div(style = 'overflow-x: scroll', DT::dataTableOutput("mr")))
                ),
                fluidRow(
                ...
                fluidRow(
                  box(title = "Survival Output Cox Proportional Hazard", status = "primary", collapsible = TRUE, collapsed = TRUE, width = NULL, div(style = 'overflow-x: scroll', DT::dataTableOutput("cox")))
                ),
                fluidRow(
...
server28 <- shinyServer(function(input, output, session){
  ...
  plot.Type<-function(pt, x, y , g, bins, fg, fgv, ip, xa, ya, gl, leglab, t, c, at, f, fv, f2, fv2, f3, fv3){
    ...
    switch(pt,
           ...
           "multibar" = {
             ...
             gg <- gg + labs(fill = legend, x = xaxis, y = yaxis) + scale_fill_manual(values = brewer.pal(3, c)) + ggtitle(title) + .theme + guides(fill=guide_legend(ncol=2))
             p <- ggplotly(gg) 
           },        
           "bar" =	{
             if(solid == FALSE){ 
           ...
           "grouped scatter" = {
             ...
             gg <- gg + labs( color = legend, x = xaxis, y = yaxis) + scale_color_manual(values = brewer.pal(3, c)) + ggtitle(title) + .theme + guides(fill=guide_legend(ncol=2))
             if(at == TRUE){
  ...
  output$mr <- DT::renderDataTable(server = F, {
    ...
    dat <- datatable(mr.df, escape = FALSE, selection = list(target = 'row+column'),  extensions = c('Buttons', 'ColReorder'), 
                     options = list(dom = 'BRrltpi', autoWidth = TRUE, lengthMenu = list(c(10, 50, -1), c('10', '50', 'All')), ColReorder = TRUE,
                                    buttons = list('copy', 'print', list(extend = 'collection', buttons= c('csv', 'pdf'), text = 'Download'), I('colvis'))), 
  ...
  output$ip.bgl <- DT::renderDataTable(server = F, {
    ...
    if(input$method == "bayesian generalized linear"){
      ...
      dt <- datatable(df, escape = FALSE, options = list(dom = 'BRrltpi', autoWidth = T, lengthMenu = list(c(10, 50, -1), c('10', '50', 'All')), ColReorder = TRUE, RowReorder = TRUE, 
                                         buttons = list('copy', 'print', list(extend = 'collection', buttons= c('csv', 'pdf'), text = 'Download'), I('colvis'))), 
  ...
  output$a1 <- DT::renderDataTable(server = F, {
    ...
    a1 <- datatable(a1.df, escape = FALSE, options = list(dom = 'BRrltpi', autoWidth = T, lengthMenu = list(c(10, 50, -1), c('10', '50', 'All')), ColReorder = TRUE, RowReorder = TRUE, 
                                          buttons = list('copy', 'print', list(extend = 'collection', buttons= c('csv', 'pdf'), text = 'Download'), I('colvis'))),
  ...
  output$a2 <- DT::renderDataTable(server = F, {
    ...
    a2 <- datatable(a2.df, escape = FALSE, options = list(dom = 'BRrltpi', autoWidth = T, lengthMenu = list(c(10, 50, -1), c('10', '50', 'All')), ColReorder = TRUE, RowReorder = TRUE, 
                                          buttons = list('copy', 'print', list(extend = 'collection', buttons= c('csv', 'pdf'), text = 'Download'), I('colvis'))), 
  ...
  output$a3 <- DT::renderDataTable(server = F, {
    ...
    a3 <- datatable(a3.df, escape = FALSE, options = list(dom = 'BRrltpi', autoWidth = T, lengthMenu = list(c(10, 50, -1), c('10', '50', 'All')), ColReorder = TRUE, RowReorder = TRUE, 
                                          buttons = list('copy', 'print', list(extend = 'collection', buttons= c('csv', 'pdf'), text = 'Download'), I('colvis'))), 
  ...
  output$ap.rf <- DT::renderDataTable(server = F, {
    if(input$method == "random forest"){
      ...
      dt.rf <- datatable(rf.df, escape = FALSE, container = sketch, options = list(dom = 'BRrltpi', autoWidth = T, scrollX = TRUE, lengthMenu = list(c(10, 50, -1), c('10', '50', 'All')), ColReorder = TRUE, RowReorder = TRUE, 
                                                                   buttons = list('copy', 'print', list(extend = 'collection', buttons= c('csv', 'pdf'), text = 'Download'), I('colvis'))), 
...
shinyApp(ui = ui28, server = server28)
