  ...
  dashboardPage( 
    ...
    dashboardBody(
      ...
      tabItems(
        tabItem(tabName = "dac", 
                fluidRow(
                  column(width = 3, box(title = "Data Overview Options", status = "warning", collapsible = TRUE, width = NULL, solidHeader = TRUE, selectInput("look", "Look at NAs, Characters, and Numeric values in Dataset", c("Total Dataset", "NAs By Row", "Data Type By Column"))), 
                         box(title = "Remove Rows/Columns in Data" , status = "primary", collapsible = TRUE, width = NULL, 
                             ...
                             conditionalPanel('input.look == "Data Type By Column"', h5(helpText("Columns Selected in Data Overview: "))),
                             h5(textOutput('lookTxt')),
                  ...
                  column(width = 9, box(title = "Data Overview Options", status = "warning", collapsible = TRUE, width = NULL , soildHeader = TRUE, div(style = 'overflow-x: scroll', DT::dataTableOutput("lookTable")))
                  )
                ), 
                fluidRow(
                ...
                fluidRow(
                  column(width = 9, 
                         box(title = "Columns Removed From Dataset",status = "primary", collapsible = TRUE, width = NULL,
                             helpText("To restore columns in data, select the clean this dataset checkbox and select 'restore to original dataset'"), 
                             uiOutput("remCol")),
                         box(title = "Rows Removed From Dataset", status = "primary", collapsible = TRUE, width = NULL, 
                             helpText("To restore rows in data, select the clean this dataset checkbox and select 'restore to original dataset'"),
                             uiOutput("remRow"))
                  ),
                  column(width = 3, 
        ...
        tabItem(tabName = "cmr",
                ...
                fluidRow(
                  box(title = "Multiple Regression Independent Variables", status = "primary", collapsible = TRUE, width = 10, div(style = 'overflow-x: scroll', DT::dataTableOutput("mr")))
                )
        ),
        tabItem(tabName = "ml",
                fluidRow(
...
server22 <- shinyServer(function(input, output, session){
  ...
  values <- reactiveValues(cleanedData = NULL)
  ...
  dataset <- reactive({
    validate(need(input$startDataset != "", "Please select a dataset"))
    validate(need(!is.null(input$datafile), "Please select a dataset or input a File"))
    if(is.null(values$cleanedData)){
      print("values$cleanData is Null")
      ...
      if(input$combine == TRUE){
        ...
        switch(input$mergeby, 
               ...
               "Merge Data by Column Value" = {
                 validate(need((input$colMerge %in% colnames(cleanedData)), "The column you for merging the datasets on is not in the original dataset, check both datasets have the column, or consider changing the column name"))
  ...
  output$p1 <- renderPlotly({
    ...
    P1 <- p1 %>% config(displayModeBar = T, showLink = T)
    print(p1)
  ...
  plot.Type<-function(pt, x, y , g, bins, fg, fgv, ip, xa, ya, gl, t, c, at, f, fv, f2, fv2, f3, fv3){
    ...
    switch(pt,
           ...
           "multibar" = {
             ...
             gg <- gg + labs(fill = legend, x = xaxis, y = yaxis) + scale_fill_manual(values = getPalette(colourCount + 9)[-(1:2)]) + ggtitle(title) + .theme + guides(fill=guide_legend(ncol=2))
             p <- ggplotly(gg) 
           },        
           "bar" =	{
             if(solid == FALSE){ 
  ...
  output$stat.res <- DT::renderDataTable(stat.tests(input$statTest, input$sig, input$x, input$y, input$group, input$factorG, input$factorGVal, input$filter, input$filterVal, input$filter2, input$filterVal2, input$filter3, input$filterVal3), 
                                         options = list(autoWidth = FALSE), class = 'table-bordered table-condensed table-striped table-compact',caption = paste(sapply(input$statTest, simpleCap), "Results" ))
  ...
  output$stat.res2 <- DT::renderDataTable(stat.tests(input$statTest2, input$sig2, input$x2, input$y2, input$group2,  input$factorG2, input$factorGVal2input$filterp2, input$filterValp2, input$filterp22, input$filterValp22, input$filterp23, input$filterValp23),
                                          options = list(autoWidth = FALSE), class = 'table-bordered table-condensed table-striped table-compact', caption = paste(sapply(input$statTest2, simpleCap), "Results" ))
  ...
  observe({
    ...
    if(input$corrVariablesStyle == "Checkbox") {
      updateCheckboxGroupInput(session, "corrVariablesCheckbox", selected = isolate(selected))
    }
  })
  ...
  observe({
    if (input$selectallCV > 0) {
      if (input$selectallCV %% 2 == 0){
        updateCheckboxGroupInput(session=session, "corrVariables", selected = input$corrVariablesCheckbox)
        
      }
      else {
        updateCheckboxGroupInput(session=session, "corrVariables", selected = c())
  ...
  observe({
    updateSelectInput(session, "IMRVariables", selected = input$corrVariables)
  })
  
  observe({
    updateSelectInput(session, "DMRVariable", choices = c(input$corrVariables))
  })
  
  observe({
    updateSelectInput(session, "DMRVariable", selected = c(input$corrVariables)[1])
  })
  ...
  observe({
    if (input$selectallML > 0) {
      if (input$selectallML %% 2 == 0){
        updateCheckboxGroupInput(session=session, "mlvariables", selected = input$mlvariablesCheckbox)
        
      }
      else {
        updateCheckboxGroupInput(session=session, "mlvariables", selected = c())
  ...
  observeEvent(input$mlButton, {
    ...
    output$ip.bgl <- DT::renderDataTable({
      ...
      if(input$method == "bayesian generalized linear"){
        ...
        dt <- datatable(df, class = 'table-bordered table-condensed table-striped table-compact', caption = "Bayesian Generalized Linear Regression Results") %>%
          formatStyle('Pr(>|t|)', color = styleInterval(0.05, c('red','black')), backgroundColor = styleInterval(0.05, c('yellow', 'white'))) %>% 
    ...
    output$a2 <- DT::renderDataTable({
      ...
      a2 <- datatable(a2.df, options = list(
        autoWidth = FALSE), class = 'table-bordered table-condensed table-striped table-compact', caption = paste("Confusion Matrix with Test Data for Model:", input$method2))
      return(a2)
    })
    output$a3 <- DT::renderDataTable({
      ...
      a3 <- datatable(a3.df, options = list(
        autoWidth = FALSE), class = 'table-bordered table-condensed table-striped table-compact', caption = paste("Confusion Matrix with Test Data for Model:", input$method3))
      return(a3)
    })
  ...
  output$ipTable <- DT::renderDataTable(var.imp(input$method, input$metric, input$number, input$repeats, input$LOOCV, input$mlButton)[[1]], options = list(autoWidth = FALSE), class = 'table-bordered table-condensed table-striped table-compact', caption = "Importance Plot Data" )
